...
| export | `deliver`, `transcode`, `compress` (`--size 10MB` two-pass), `audiogram`, `slideshow` |
| captions / mute | `caption` (`--mode burn` social safe-zone, or `--mode mux`) |
| hook text | `title` |
| cover still | `cover` |
| speech / music | `jumpcut`, `denoise`, `music`, `replace` (`--audio` swap the track), `loudnorm`, `volume` |
