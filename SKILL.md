...
| voice all over the place | `leveler` (`--threshold`/`--ratio`/`--makeup` — `acompressor`) |
...
| solid color card / backplate | `solid` (`--color`/`--size`/`--dur`, optional silent track) |
...
| freeze a beat / outro hold | `freeze` (`--ease` swoop, `--at T --dur D`, or `--end D`) |
