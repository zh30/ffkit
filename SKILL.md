...
| captions / mute | `caption` (`--karaoke` word reveal, `--box-color` card, `--mode burn` social safe-zone, `--chunk N` word groups, `--align` line alignment, or `--mode mux`), `--fade` |
...
| watch loudness while it plays | `meter` (`--size`, `--meter 9|18` — EBU R128 video; podcast/voice QC) |
...
| 3-2-1 intro countdown | `countdown` (`--from`, `--each`, `--go`, `--at`, `--text`, `--position`, `--beep` + `--tone` Hz) |
...
| rolling end credits | `scroll` (`--text`/`--file`, `--at`, `--dur`, `--align` left/right — text rolls bottom→top) |
...
| two-camera angle switching | `multicam` (`A B --at t1,t2,...` — run `align` first if the takes aren't synced; `--keep-audio` stays on cam A, `--transition` soft cuts) |
...
| burn an .srt/.ass into pixels | `subs` (`--burn subs.srt` — libass), `--box` plate, `--shadow` depth, `--rate` drift fix, `--safe` social zone; `--convert` srt↔vtt |
...
| solid color card / backplate | `solid` (`--color`/`--size`/`--dur`, optional silent track), `--noise` grain, `--text` card text (`--wrap` folds), `--fade` card fades |
...
| repeat just the funny bit | `loop` (`--from`/`--to`/`--times`, `--fade` seamless joints) |
