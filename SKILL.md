...
| trim / join | `cut` (`--black` excises blackdetect dead-air stretches ≥0.3s), `concat` (`--transition` any xfade — all 42 named 4.4 transitions (comma list picks one per joint), `--copy` forces the lossless stream-copy join or fails on spec mismatch (the auto-pick silently re-encodes mismatched inputs — --copy turns it into a gate), `--repeat N` repeats the whole joined sequence N times (loop-intended assemblies — intro+loop+outro patterns, transitions hit every repeat seam), N clips, `--level -14` loudnorms each, `--gap N` black+silence between clips, `--audio-fade N` boundary fades at joints, `--list manifest.txt` reads clip paths from a file, `--chapters` titles each clip as a container chapter at its join — audiobook/podcast assembly (relative to the list's dir)), `split` (`--every` story chunks, `--at` chapter points, `--chapters` embedded marks, `--black` dead-air → one part per keep, `--copy` lossless stream-copy split — instant, boundaries snap to next keyframe), `rough` (list speech islands, then `-o` to assemble, `--merge N` merge close keeps, `--by-scene` split at cuts) |
...
| export | `deliver` (`--platform youtube|square|xhs|wechat|douyin|kuaishou|bilibili|pinterest|x|linkedin|vimeo|bluesky|threads|mastodon|circle|canvas|snapchat|weibo|whatsapp|twitch|discord|shopify|amazon|etsy|rumble|instagram|facebook|kick|line|vk|dailymotion|odysee|trovo|substack|triller|lemon8|niconico|soop|xigua|peertube|floatplane|nebula|chzzk|douyu|huya|likee|moj|josh|weverse|kwai|snackvideo|udemy|coursera|teachable|kajabi|patreon|skillshare|thinkific|podia|learnworlds|gumroad|wistia|domestika|steam|itch|shopee|lazada|taobao|dlive|minds|telegram|tidal|deezer|qobuz|yandexmusic|napster|joox|soundcloud|mixcloud|audiomack|bandcamp|vevo|roku|plex|iqiyi|youku|wetv|viki|crunchyroll|funimation|mgtv|bigo|nimo|tumblr|dribbble|behance|flickr|zhihu|kakao|naver|coub|imgur|9gag|streamable|viddsee|rutube|ok|zen|openrec|twitcasting|showroom|fc2|tving|wavve|watcha|vidio|mewatch|tver|abema|hotstar|jiotv|sonyliv|mxplayer|zee5|showmax|shahid|truthsocial|gettr|parler|locals|utreon|caffeine|qq|tubi|pluto|dazn|espn|hulu|u-next|gyao|netflix|disney|max|peacock|paramount|appletv|primevideo|globoplay|viaplay|joyn|raiplay|atresplayer|itvx|crave|spotify|apple|amazonmusic|iheartradio|pandora|castbox|podbean|stan|mycanal|skygo|movistar|viu|voot|clarovideo|nhk|arte|tv2play|npostart|rtve|tvp|voyo|wakanim|adn|laftel|aniplus|hidive|retrocrush|bstation|ard|zdf|nrk|svt|dr|cbc|sbs|tf1|francetv|mediaset|channel4|tenplay|nowtv|srf|fubo|sling|philo|directv|xumo|vidgo|frndly|iplayer|my5|britbox|acorntv|shudder|showtime|starz|reddit|zillow|ebay|walmart|poshmark|whatnot|kanopy|mubi|criterion|curiositystream|magellantv|tinder|bumble|hinge|brightcove|jwplayer|kaltura|sproutvideo|vidyard|uscreen|vdocipher|mercari|vinted|depop|carousell|olx|jellyfin|emby|onlyfans|fansly|fanbox|cameo|subscribestar|kofi|buymeacoffee|buzzsprout|captivate|transistor|redcircle|sounder|acast|spreaker|bandlab|distrokid|tunecore|amuse|cdbaby|symphonic|landr|sharechat|chingari|vmate|blim|vix|irokotv|starzplay|pearvideo|haokan|miaopai|acfun|toutiao|baijiahao|ifeng|weishi|huoshan|quanmin|meipai|migu|pptv|letv|pdd|jd|vip|kocowa|rakuentv|iwanttfc|hoichoi|17live|pococha|mirrativ|mxtakatak|roposo|boomplay|sohu|pixelfed|artstation|clapper|younow|meesho|bulbul|fanvue|temu|shein|aliexpress|flipkart|zalando|coupang|mercadolibre|nba|nfl|mlb|nhl|fifa|ufc|wwe|tsn|sportsnet|beinsports|skysports|tntsports|foxsports|cbssports|eurosport|kayo|optussport|supersport|astro|willow|premier|laliga|bundesliga|seriea|ligue1|mls|championsleague|mildom|mtv|bet|vh1|comedycentral|nickelodeon|cartoonnetwork|adultswim|cnn|abc|nbc|cbs|foxnews|aljazeera|bbcnews|tmall|noon|nykaa|daraz|jumia|tiktokshop|quikr|dubizzle|wallapop|subito|kleinanzeigen|blocket|tradera|leboncoin|dropbox|box|onedrive|gdrive|mega|wetransfer|sendanywhere|loom|tella|screenpal|vidcast|msstream|panopto|sharepoint|zoom|webex|gotomeeting|bluejeans|ringcentral|hopin|airmeet|shutterstock|pond5|artgrid|storyblocks|videvo|motionarray|dissolve|rightmove|zoopla|realtor|redfin|domain|immoscout|idealista|autotrader|cargurus|carvana|carwow|mobilede|autoscout|copart|airbnb|booking|expedia|hotels|tripadvisor|agoda|vrbo|ubereats|doordash|deliveroo|grubhub|swiggy|zomato|meituan|robinhood|etoro|webull|coinbase|binance|kraken|public|okcupid|match|grindr|eharmony|zoosk|badoo|pof|medium|ghost|wordpress|squarespace|wix|webflow|framer|newgrounds|deviantart|vsco|smugmug|zenfolio|9now|7plus|plutotv|freevee|fubotv|globo|pbskids|boomerang|cartoonito|draftkings|fanduel|bet365|williamhill|betfair|skybet|paddypower|pga|atp|wta|icc|f1|motogp|nascar|podcast|audiobook`), `--timescale N` pins the mp4 video clock (`-video_track_timescale` — broadcast pickup specs that lock the movie clock), `transcode` (`--copy-audio`, `--copy-video` audio-only re-encode, `--vbitrate` peak cap, `--abitrate` audio bitrate, `--preset mp3`/`aac`/`wav`/`flac`/`opus` audio-only, `--preset ffv1` lossless .mkv archival master (video ffv1 + audio flac — museum-grade intermediate), `--preset apng` full-color animated PNG loop (.apng — stickers/reactions where gif's pal8 bands), `--preset mpeg2` MPEG-2 + MP2 DVD/broadcast legacy master (.mpg/.vob — set-top players, TV ingest), `--preset mpeg1` MPEG-1 + MP2 VCD-era legacy master (.mpg — the oldest digital video format still in playback), `--preset xvid` Xvid/MPEG-4 + MP3 in .avi (legacy-rip master — DivX-era players & projectors), `--preset wmv` WMV2+WMA in .wmv/.asf (Windows Media-era master — corporate training archives, old PowerPoint video), `--preset msmpeg4` MS-MPEG4 v2+MP3 in .avi (MP42 tag — the pre-DivX Windows codec: ME-era screen captures, ancient players), `--preset gpp` H.263+AMR-NB in .3gp (feature-phone master — MMS-era mobile video; snaps to the 5 legal h263 canvas sizes letterboxed, audio forced 8kHz mono), `--preset flv` FLV1+MP3 in .flv (Flash-era web master — YouTube 2005-era uploads), `--preset theora` Theora+Vorbis in .ogv/.ogg (open-web master — pre-WebM HTML5 video, Wikimedia embeds), `--preset ogg` audio-only Ogg Vorbis (.ogg — open-web music/podcast upload), `--preset alac` ALAC in .m4a (Apple Lossless archive — lossless music for the Apple ecosystem, no quality loss vs wav), `--preset dv` DV25 in .dv/.avi (camcorder-tape master — MiniDV/DVCAM archives; snaps to the NTSC-legal 720x480@30000/1001 yuv411p + PCM 48kHz stereo spec, tuning flags refused), `--preset mjpeg` Motion JPEG+MP3 in .avi/.mov (NLE-era editing format — every frame an intra JPEG for frame-accurate scrub masters), `--preset amv` AMV+ADPCM in .amv (Chinese handheld-player master — MP3/MP4 players; fixed spec like dv: snaps to 160x120@25fps + ADPCM 22050Hz mono letterboxed, tuning flags refused), `--preset qtrle` QuickTime Animation RLE in .mov (lossless animation/screencast master — rgb24, argb with --alpha), `--preset v210` uncompressed 10-bit 4:2:2 broadcast master + PCM in .mov (bitrate flags meaningless — spec ingest), `--preset huffyuv`/`utvideo`/`ffvhuff` lossless capture intermediates + PCM (NLE-era edit masters — faster than ffv1; .avi for huffyuv/utvideo, .mkv for ffvhuff/huffyuv), `--preset cinepak` Cinepak+PCM in .mov/.avi (CD-ROM-era codec — mid-90s QuickTime/Windows video, Myst-era game archives), `--preset svq1` Sorenson Video 1+PCM in .mov (QuickTime 2-4 era web video — the pre-Flash internet standard), `--preset zmbv` Zip Motion Blocks Video+PCM in .avi (DOSBox-era screencast recordings — game-capture archives), `--preset v410` uncompressed 10-bit 4:4:4 + PCM in .mov (v410 yuv444p10le — finishing-suite interchange, the colour-managed master colorists/QC pass around), `--preset ayuv` uncompressed 8-bit 4:4:4:4 + PCM in .mov (ayuv yuva444p — always carries alpha; motion-graphics interchange when prores4444 is too compressed), `--preset rv10`/`rv20` RealVideo + RealAudio 1.0 in .rm (late-90s dial-up streaming — ra_144 pinned 8kHz mono), `--preset r210` uncompressed 10-bit RGB + PCM in .mov (r210 gbrp10le — the RGB member of the finishing-suite uncompressed trio), `--preset v308` uncompressed 8-bit 4:4:4 + PCM in .mov (v308 yuv444p — the 8-bit sibling of v410), `--preset rpza` Apple Video + PCM in .mov (rpza rgb555le — the original QuickTime 1.x codec), `--preset speedhq` NewTek SpeedHQ + PCM in .mov/.avi (NDI-era NLE intermediate — TriCaster/NDI capture masters), `--preset roq` id RoQ + RoQ DPCM in .roq (Quake III-era game video — dims snap to powers of two letterboxed, audio pinned 22050Hz), `--preset snow` Snow wavelet + PCM in .mkv (ffmpeg's native experimental codec — lossless-quality archival), `--preset flashsv`/`flashsv2` Flash Screen Video + MP3 in .flv (screen-recording era), `--preset msvideo1` Microsoft Video 1 + MP3 in .avi (the oldest Windows codec, palettized), `--preset cljr` Cirrus Logic AccuPak + PCM in .mov (early-90s QuickTime), `--preset ac3`/`eac3` Dolby Digital/DD+ audio-only in .ac3/.eac3 (broadcast ATSC + streaming-era surround spec), `--preset tta` TTA lossless audio-only in .tta, `--preset dca` DTS audio-only in .dts (disc-era surround — experimental encoder, shipped with -strict -2), `--preset raw` rawvideo + PCM in .avi/.mkv (bit-exact interchange — keeps the source pixel format, no conversion), `--preset aiff` AIFF + PCM-BE audio-only in .aiff (Apple-era lossless master), `--preset pcm24`/`pcm32f` 24-bit / 32-bit-float WAV audio-only (studio master + DAW interchange), `--preset mulaw` G.711 µ-law audio-only in .au pinned 8kHz mono (telephony/IVR spec — refuses --ar/--channels), `--preset adx` CRI ADX in .adx (Sega-era game audio), `--preset adpcm` IMA-ADPCM in .wav (classic game-engine audio), `--preset alaw`/`mulaw` G.711 A-law/µ-law in .au pinned 8kHz mono, `--preset speex` Speex in .spx, `--preset pcm8` 8-bit PCM in .wav, `--preset adpcmms` MS ADPCM in .wav, `--preset g722` G.722 in .wav pinned 16kHz mono, `--preset ra144` RealAudio 1.0 in .rm pinned 8kHz mono, `--preset nelly` Nellymoser in .flv; `--preset wv` WavPack lossless in .wv, `--preset mp2` MPEG Layer II in .mp2 (broadcast/DAB audio), `--preset caf` PCM-BE in .caf (Apple Core Audio interchange), `--preset w64` 24-bit PCM in .w64 (Sony Wave64 — >4GB recordings), `--preset voc` PCM in .voc (DOS-era game audio), `--preset aptx`/`sbc` Bluetooth codecs in .aptx/.sbc (aptX resamples to its native 48k), `--preset g723` G.723.1 in .tco pinned 8kHz mono, `--preset hap`/`hapq` Vidvox HAP DXT-texture + PCM in .mov/.avi (live-visual codec — Resolume/TouchDesigner/VJ ingest; `--alpha` upgrades hap → Hap Alpha DXT5, `hapq` is the YCoCg hi-q variant), `--preset cfhd` GoPro CineForm HD + PCM in .mov/.avi (action-cam NLE intermediate, yuv422p10le), `--preset vc2` SMPTE VC-2/Dirac + PCM in .mov (BBC broadcast intermediate — probe reads codec `dirac`), `--preset magicyuv` MagicYUV lossless + PCM in .avi (NLE-era fast lossless — huffyuv/utvideo sibling), `--preset r10k` AJA Kona 10-bit RGB + PCM in .mov (gbrp10le capture-card master), `--preset truehd`/`mlp` Blu-ray/HD-DVD lossless audio-only in .thd/.mlp (experimental encoders — ffkit passes -strict -2 for you); `--preset h261`/`h263` raw telecom elementary (videoconference test vectors — snaps to the legal CIF-family canvas letterboxed, video-only), `--preset avui` Avid Meridien uncompressed + PCM in .mov (broadcast capture-card ingest — snaps to 720x486, experimental -strict -2); `--preset ts` H.264+AAC in .ts/.m2ts MPEG-TS (broadcast ingest, IPTV/DVB archive), `--preset mxf` XDCAM/OP1a master (mpeg2video 4:2:2 + 48kHz stereo PCM — spec-pinned, refuses audio knobs), `--preset ivf` VP9 in .ivf elementary (video-only — MSE/Shaka test vectors; `--crf` is true constant-quality, `-b:v 0` auto), `--preset gxf` Grass Valley GXF broadcast-server interchange (source rate snaps PAL 720x576@25 or NTSC 720x480@30000/1001 — spec-pinned mono PCM), `--preset wtv` Windows Media Center recording (mpeg2video+mp2), `--preset smjpeg` MJPEG+PCM in .smjpg (Loki/SDL-game FMV — smpeg-era engines);; rv10/rv20 now carry RealAudio audio (not video-only), `--tune film|animation|grain|zerolatency|fastdecode|stillimage|psnr|ssim` x264 content-shape tune (h264/proxy only), `--gop N` keyframes every N frames — ingest specs cap GOP size and short GOPs scrub/seek faster, `--profile baseline|main|high` x264 encode profile, `--level 4.1` x264 encode level, `--bf N` max B-frames, `--tune grain` & friends x264 content-shape tune (device-compat ingest specs — baseline+low level+0 B-frames for old phones/car/kiosk players; h264/proxy only)), `compress` (`--size 10MB` — or a named cap: discord/nitro/whatsapp/gmail/messenger/wechat — two-pass, `--crf` quality one-pass, `--res` downscale, `--fps` rate cap, `--ar` audio resample, `--channels N` mono-cap speech), `audiogram` (`--progress`, `--mode`/`--color`), `slideshow` (`--motion kenburns`, `--transition`, `--list manifest.txt` curated order), `split`, `--subs` captions, `--preset prores`/`dnxhd`, `--target` platform sizes |
...
| podcast feed pack (−16 LUFS spec) | `deliver --platform podcast` (m4a AAC 128k/48k, loudnorm to feed spec; works on audio-only sources, `--cover art.png` embeds the Apple/Spotify cover) or `--platform audiobook` (m4b AAC 96k — Apple Books/Audible); packs take `--chapters marks.txt` (real container chapters from the YouTube-format `mm:ss title` list `chapter --yt` exports — Apple Podcasts seek stops on audio packs; mp4 container chapters YouTube reads as timeline markers on video packs) and feed tags `--title/--author/--album/--genre/--comment`; any platform: `--preview SEC` renders just the pack's head for approval QC, `--logo mark.png` burns a corner watermark during the pack render (`--logo-position tl/tr/bl/br`, `--logo-opacity`), `--intro/--outro clip.mp4` bakes a channel bumper and CTA card onto every export (normalized to the platform canvas); `--maxrate`/`--bufsize` CBR rate-cap pair on the pack encode (platform ingest envelopes — bufsize defaults to 2x maxrate; `conform` takes the same pair); `--profile`/`--level`/`--bf` device-compat trio on the pack h264 (baseline+low level+0 B-frames for car/kiosk players); Chinese feeds: `--platform douyin|kuaishou` (9:16 1080x1920) / `bilibili` (16:9 1920x1080); `--program N` packs one service out of a multi-program transport stream (broadcast pickup ingest — `probe.programs[]` lists services; the pack takes that service's first video/audio member) |
...
| web-embed HLS package | `hls` (`--seg` seconds, `--single` one-file, `--copy` repack, `--poster` writes poster.jpg, `--poster-at T` picks the frame, `--encrypt`/`--key HEX`/`--key-uri URI` AES-128 segments + key.bin/key.info, `--rekey` rotates the key file per segment, `--temp` atomic tmp→rename writes, `--round-durations` whole-second EXTINF) → dir/`index.m3u8` + `seg_*.ts`; `--ladder 1080,720,480` → ABR variant playlists + `master.m3u8`; `--audio-only` podcast HLS; `--video-only` muted/preview renditions; `--fmp4` CMAF `.m4s` segments; `--live` sliding-window playlist (players join mid-write — newest `--live-window N` segments only, no endlist); `--start N` resumes segment numbering after a restart, `--epoch` seeds it from the epoch clock (24/7 channels), `--date` stamps EXT-X-PROGRAM-DATE-TIME on segments, `--discontinuity` marks a feed restart, `--time-names` names segments by wall-clock time (archive the recording by time), `--independent` tags EXT-X-INDEPENDENT-SEGMENTS + forces a keyframe per segment, `--iframes` tags EXT-X-I-FRAMES-ONLY (trick-play scrub previews), `--base-url URL` prefixes every playlist segment entry — serve segments from a CDN while the manifest stays local, `--name PFX` prefixes segment filenames (`v-seg_…` on every scheme — several packs share one dir), `--program N` packages one service out of a multi-program transport stream — broadcast pickup ingest (probe.programs[] lists services; works with --ladder — the ladder maps the program's own member streams)); `dash` DASH-pack → dir/manifest.mpd + init-/seg-*.m4s (`--seg`, `--copy` repack, `--single` one byte-range file per representation, `--webm` vp9+opus, `--window N` sliding live manifest, `--ladder 1080,720,480` → N video Representations at tiered bitrates in one AdaptationSet — adaptive DASH; `--streaming` writes a moof fragment per frame (low-latency DASH prep), `--frag SEC` writes a moof fragment per SEC inside each segment (trick-play prep — between per-frame and whole segments), `--sidx` writes a global SIDX box into the `--single` byte-range file (HTTP range seeking), `--name PFX` prefixes segment filenames (`v-init-…`/`v-seg-…` — several packs share one dir), `--utc URL` UTCTiming clock for live manifests (players sync wall-clock to the live edge — pair `--window`), `--program N` packages one service out of a multi-program transport stream — broadcast pickup ingest (probe.programs[] lists services; works with --ladder via the program's member streams)) |
...
| normalize mixed footage for concat | `conform` (`--size WxH`, `--fps`, `--lufs`, `--pad` letterbox color + `--anchor`, `--blur` blurred fill, `--even` floors odd px dims — phone captures can't take yuv420p, `--ar HZ` resample rate (48k broadcast default; 44100 podcast/CD, 96000 masters), `--channels 1` mono masters, `--program N` conforms one service of a multi-program transport stream (broadcast pickup — `probe.programs[]` lists services; first video+audio members), `--profile/--level/--bf` device-compat trio on the spec pass (baseline+low level+0 B-frames for car/kiosk players), `--rotate 90|180|270` transposes the picture during the spec pass (portrait phone footage → landscape spec in one encode — runs before `--size` so the canvas math sees the rotated frame), `--timescale N` pins the mp4 video clock (mp4/mov targets — ingest specs that lock the movie clock) |
...
| burn an .srt/.ass into pixels | `subs` (`--burn subs.srt` — libass), `--box` plate, `--shadow` depth, `--margin` px, `--rate` drift fix, `--from/--to` cue window, `--safe` social zone; `--convert` srt↔vtt (+ `.ass` both directions — Style/Format columns honoured) ; `--burn-si N` burns embedded track N; `--encoding gbk` legacy charset; `--sort` reorders out-of-order cues, `--fix-overlaps` clamps each cue's end to the next start (overlap repair — pairs with --sort when times are scrambled), `--dedupe` drops exact repeats, `--dedupe-text` drops a cue repeating the previous kept cue's text (Whisper repeat-line artifacts — timings may differ, compared case/space-insensitive), `--fix-cps N` stretches each over-CPS cue until it reads at N chars/sec (auto-fix for the --cps gate — capped at the next cue's start; extras: stretched) + `--fix-lines N` splits each over-N-line cue into sequential cues at line boundaries (`lines_split`) + `--speakers` reports the transcript cast list (extras: speakers[] + speaker_count — pairs with --strip-speakers) + `--stats` reports transcript stats (extras: cues/words/chars/span_secs/median_dur_secs — script-length QC for read pacing) — .srt cue-file hygiene for broken exports; `--min-dur SEC` extends flash-text cues up to SEC (caps at the next cue's start), `--min-gap SEC` trims the earlier cue's tail when the gap before the next cue is shorter (broadcast spec ~2 frames), `--join SEC` merges adjacent cues closer than SEC into one (auto-transcript over-fragmentation repair — Whisper-style fragments join into sentence-length cues); `--cps N` caption-speed gate (`over_limit`/`worst_cps` — Netflix-style readability spec), `--max-lines N` reports cues with more than N text lines (broadcast spec is 2); `--replace OLD,NEW` find/replaces cue text (rename a character across the file), `--strip-speakers` drops `[NAME]`/`<NAME>`/`ALL-CAPS:` speaker labels (auto-transcripts), `--wrap N` rewraps cue lines at N chars (portrait phones); `--append b.srt` joins a second subtitle file where the first file's cues end (post-`concat` transcripts), `--convert` to `.txt` exports a plain-text transcript, to `.ass` writes minimal styled ASS (Aegisub/anime-pipeline handoff), to `.lrc` writes synced lyrics (transcript → karaoke/music-player lyrics — the next line's timestamp acts as each line's end), to `.ttml`/`.dfxp` writes minimal TTML/DFXP (broadcast/Netflix subtitle exchange — `.ttml`/`.dfxp` input parses `<p begin end>` cues back too), `.sbv` reads/writes YouTube SubViewer captions (`H:MM:SS.mmm,H:MM:SS.mmm` headers — Studio-editable uploads round-trip), `.csv` reads/writes `start,end,"text"` rows (quoted cells keep commas, quotes and line breaks — edit cues in Sheets/Excel and convert back); `.sub` MicroDVD `{f}{f}text` input reads frame-based subs (a `{1}{1}fps` header line wins, else `--fps`) — legacy anime/TV archives, and `.sub` output writes them back at `--fps` (default 25); `.mpl` MPL2 `[s][e]text` decisecond subs read/write (Polish legacy archives — `|` line breaks); `.smi` SAMI `<SYNC Start=ms>` read/write (Windows Media-era captions — a cue holds until the next SYNC; `.scc` Scenarist closed-caption input decodes 608 pairs via ffmpeg's demuxer — read-only, muxer is passthrough so srt→scc stays out; `.stl` Spruce broadcast + `.rt` RealText + `.mps` MPlayer start+duration inputs decode the same way); `.pjs` Phoenix `start,end,"text"` input reads decisecond rows (legacy anime-fansub archives — parsed in ffkit); `.psb` PowerSub `{hh:mm:ss.mmm}{hh:mm:ss.mmm}text` input (the OTHER brace format — MicroDVD braces count frames, PSB braces carry timestamps; parsed in ffkit); `.sub` input sniffs the shape: `{f}{f}` MicroDVD lines parse as frames (a missing rate still reports --fps), anything else goes through ffmpeg's subviewer demuxer (reads SubViewer v1+v2) — the extension is ambiguous in the wild; written cues land `<SYNC Start=ms><P Class=ENCC>` per cue); `--split 3,8` cuts an .srt at comma timestamps into `stem_0.srt`/`stem_1.srt`… parts — cues re-timed per part (splitting a transcript to match `split`/`cut` parts); `--resync O1,O2,N1,N2` remaps cue times linearly through two sync points (offset+drift in one pass — retiming subs exported for a different cut), `--find needle` keeps only cues containing the text (locate every 'um'/phrase spoken — cut or caption around them); `--strip-tags` drops inline markup from cue text — `{\…}` ASS override blocks (`<…>` tags are already dropped on load) for transcripts that burn literal markup; `--strip-sdh` drops `[SOUND]`/`(door creaks)`/`♪` annotations — dialogue-only transcript for burning; `--strip-emotes` strips emoji/pictographs (broadcast caption paths reject them — 608/708-safe transcript); `--rtl` wraps every cue-text line in U+202B..U+202C RTL marks (Arabic/Hebrew captions render mirrored punctuation in players without them — MENA subtitle delivery; extras: rtl_wrapped); `--clip F,T` keeps cues overlapping a window, clamps the edges and re-times to 0 (grab the subtitle chunk for a cut segment — `end` ok for T), `--drop F,T` drops the window's cues and re-times the tail left by (T-F) — the `cut --drop` counterpart for transcripts, `--move N,T` re-seats cue N to start at T (nudge one mis-timed cue — N is the input file's numbering, duration kept) |
