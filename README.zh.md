...
| `concat` | 拼接 N 段（全部 42 种 4.4 命名 xfade `--transition`，逗号列表逐接缝选转场）；`--copy` 强制无损流拷贝拼接，规格不一致直接失败（保证成片没重编码的质检——规格不符时自动路径会静默重编码）；`--level -14` 先统一各段响度；`--gap N` 段间插入黑场+静音 ；`--audio-fade N` 接缝处音频淡化（边界淡化，时长/同步不变）；`--list manifest.txt` 从清单文件读取片段路径（相对路径按清单所在目录解析） |
