...
| `frames` | 每 `--every`、`--at` 秒抽一帧（`end` = 最后一帧）、`--count` 均布 N 帧、`--nth N` 每隔 N 帧抽一帧（数据集/质检采样）、`--number N` 精确抽第 N 帧（0 基解码序——按帧号定点抓坏帧，VFR 上 --at 的时间换算会漂移）→ `stem_001.png…`（`--width` 缩放）；`--untile CxR` 把每帧拆成瓦片静帧（还原宫格图/马赛克） |
