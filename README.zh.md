...
| `audiogram` | 波形视频 | `--mode`、`--scale` 幅度、`--split` 分声道、`--text`、`--bg`、`--progress` 进度条 ，`--subs` 烧字幕；`--mode spectrum` 频谱条 |
...
| `scroll` | 片尾滚动字幕（`--text`/`--file`、`--at`、`--dur`、`--size`、`--color`、`--font`、`--align` 左/右对齐）；`--mode ticker` 底部新闻条 |
