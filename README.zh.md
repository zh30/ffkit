...
| `audiogram` | 波形视频 | `--mode`、`--scale` 幅度缩放、`--text`、`--bg`、`--progress` 进度条 ，`--subs` 烧字幕；`--mode spectrum` 频谱条 |
...
| `spectrogram` | 音频频谱图 → PNG（`--size`），清理前先看嗡鸣/噪声（`--color` magma/viridis…、`--scale` lin/sqrt…）（`--at/--dur` 只画片段） |
