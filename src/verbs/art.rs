...
pub fn run(args: ArtArgs, g: &Globals) -> Result<Contract, Error> {
    ...
    argv.extend(["-i".to_string(), args.image.display().to_string()]);
    ...
    let inputs: Vec<&Path> = vec![&args.input, &args.image];
    let mut c = engine::write_job("art", &inputs, &args.output, vec![argv], g)?;
    c = c.with_extra(json!({ "art": paths::display(&args.image) }));
    Ok(c)
}
