...
| `timer` | On-screen running clock (`--position`, `--format ms`, `--box-color` card)  (`--format`, `--box-color`, `--down` countdown, `--start` seed the readout) — `--at` accepts `end`; `--opacity` ghost HUD; `--tc HH:MM:SS:FF` burns a running timecode (dailies/review copies; `;` before FF = drop-frame display intent), `--clock` burns the local wall clock HH:MM:SS (event/sports overlays), `--date` burns the local calendar YYYY-MM-DD (air-date/archive overlay — static; prefixes the --clock readout) |
