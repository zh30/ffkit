...
| `chapter` | Embed chapter marks at `TIME|TITLE` or `--import` a marks file (YouTube `H:MM:SS Title` lines ok); `--auto` / `--export` ffmeta / `--yt` description lines / `--cue` CUE sheet / `--podcast` Podcasting 2.0 JSON chapters (audiobook/podcast players) / `--lrc` synced-lyrics cue file / `--vtt` WebVTT chapter track for web `<track kind="chapters">` nav; `--spread N` even-grid marks (`--titles a,b,c` names them — uniform TOC for long episodes); `--import` auto-reads .json/.cue/.lrc/.vtt files too; `--scenes` scdet scene-cut detection — marks every cut like `split --scenes` finds (auto-TOC for unmarked masters); `--list`; `--remove`; `--shift` re-times marks |
