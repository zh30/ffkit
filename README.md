...
| `probe` | Duration, size, codecs, channels, `av_desync_ms` lip-sync offset, `timecode` container TC, `has_alpha`, `tags` metadata audit, `streams` per-track table (index/kind/codec/lang/title/sample_rate/profile/fps/duration/bit_rate/default/forced), `rotation` display-matrix QC, `start_time` earliest stream start (negative/odd capture starts — repair via `remux --offset`), `chapter_count` embedded-TOC count (QC that a deliverable carried its chapters — `chapter --list` shows the marks) |
