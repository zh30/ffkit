...
| `deliver` | One-shot platform pack (Reels / TikTok / Shorts 9:16, `square` 1:1 grid, `youtube` 16:9, `xhs` 小红书 3:4 1080x1440, `wechat` 视频号 6:7 1080x1260; −14 LUFS; `--fps 60` high-frame-rate uploads, `--crf` quality, `--subs file.srt` burns captions in one pass); `--platform podcast` audio-only feed pack (m4a AAC 128k/48k, −16 LUFS spec); `--to rtmp://…`/`tcp://`/`udp://` pushes the rendered pack straight to ingest (premieres) |
...
| `live` | Push the clip live to an ingest endpoint: `--to rtmp://…`/`rtmps://`/`tcp://`/`udp://` (real-time `-re` pacing, x264/aac), `--loop` forever (24/7 streams, premiere replays), `--vbitrate`/`--abitrate` |
