...
| `probe` | Duration, size, codecs, channels, `av_desync_ms` lip-sync offset, `timecode` container TC, `has_alpha`, `tags` metadata audit |
...
| `meta` | Container tags (`--title`/`--artist`/`--album`/`--genre`/`--date`/`--track`/`--disc`/`--composer`/`--bpm`/`--lyrics file.lrc`/`--copyright`/`--comment` + `--album-artist`/`--show`/`--season`/`--episode`/`--network` TV/podcast feed set + `--creation-time`/`--location` archive & geo stamps + `--media-type` iTunes kind + `--gapless`) + `--rotate`, `--clear`, stream-copy |
