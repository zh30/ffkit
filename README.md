...
| `chapter` | Embed chapter marks at `TIME|TITLE` or `--import` a marks file (YouTube `H:MM:SS Title` lines ok); `--auto` / `--export` ffmeta / `--yt` description lines / `--cue` CUE sheet / `--podcast` Podcasting 2.0 JSON chapters (audiobook/podcast players); `--list`; `--remove`; `--shift` re-times marks |
