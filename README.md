...
| `subs` | Extract (`--stream`, `--all`)/burn/mux subtitles (`--shift` (±N; `--from`/`--to` bounds it)/`--merge`/`--rate`, burn style + `--outline`/`--box` plate/`--align`/`--margin` px/`--from`/`--to` window (`end`/`end-N` ok), `--safe`); `--convert` .srt↔.vtt; `--case` cue text; `--burn-si N` burns embedded track N; `--encoding gbk` legacy charsets; `--sort`/`--fix-overlaps`/`--dedupe` .srt cue hygiene (reorder out-of-order cues, clamp overlapping ends to the next start, drop exact repeats — broken-export repair) |
