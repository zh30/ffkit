...
| `remux` | Container swap, no re-encode (`-c copy` + faststart on mp4/mov); `--audio` rips the track, `--video` video-only repack, `--aspect 16:9` display-AR fix, `--frag` fragmented MP4 (moof/mfra — playable while still being written, HLS/DASH pipelines), `--no-subs` drops subtitle/data streams (clean deliverable) |
