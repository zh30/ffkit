...
Researched 2026-09-22 against **ffkit 0.36.0** (`main` + round-11 branch). Same-day research base as the 0.26.0/0.27.0 rounds below — refreshed priorities only, no new sources needed. This note is not a restatement of landed work — the tail lists what not to redo.
...
*(round 11: `split --scenes`, `cutsil`, `grid --audio`. round 10: `replace --duck`, `pitch`, `grade --grain`. round 9: `autocrop`, `sheet`, `title --at`. round 8: `boomerang`, `chapter`, `zoom --at/--dur`, `key --despill`)*
