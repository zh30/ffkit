...
- `-metadata isrc/license/publisher` land on mp3/flac/mkv/ogg but the mp4/m4a/mov whitelist **drops them silently** — same pattern as `make`/`model` (.mov-only keys). Tag music releases on the open containers, or carry the data in `description`/`copyright` for mp4 packs.
