...
- **`fps=N` emits round(duration×fps) frames** — on a clip shorter than `2/rate` it writes ZERO files silently (the command exits 0, the verify fails). `frames` detects `every ≥ 2×duration` and falls back to the first frame; raw ffmpeg users hit the same floor.
- **Bitmap subtitle encoders are bitmap-in only** — `dvdsub`/`xsub`/`dvbsub` can't encode from text: "Subtitle encoding currently only possible from text to text or bitmap to bitmap". `.sup` export and xsub-in-AVI muxing are unreachable from `.srt`; rasterize via `subs --burn` or author with a DVD tool.
- **`transcode --preset h261`/`h263` write raw elementary streams** — no container, no audio; both snap to the legal CIF-family picture sizes letterboxed (H.261: 176x144/352x288; H.263: the five H.324 sizes). `avui` is the same fixed-spec shape: only 720x486/720x576 exist.
...
- **GXF carries MONO audio tracks only** — the muxer refuses stereo ("only mono tracks are allowed"), and it only accepts PAL/NTSC canvases. `--preset gxf` folds stereo to `-ac 1` and snaps canvas+rate by source fps (PAL 720x576@25 / NTSC 720x480@30000/1001); probe reports r_fps double (50/59.94) because the muxer marks fields — tbr is the real rate.
- **`hls --temp` writes `seg_NNN.tmp` then renames** — readers on a live playlist never open a half-written segment; on 4.4 it applies to the playlist file too (atomic publish both ends).
- **smjpeg declares no extension mapping** — ffmpeg can't infer the muxer from `.smjpg`; `--preset smjpeg` emits `-f smjpeg` explicitly (the only preset that must force its container format).
- **tsid/onid write fine on 4.4 but ffprobe won't show them** — `remux --tsid/--network-id` land in the PAT/SDT section headers (DVB ingest reads them), yet `probe.programs[]` only reports `num`/`service_name`/`service_provider`; verify the mux IDs in a TS analyser, not in probe.
- **`-update_period`/`-target_latency`/`-min_playback_rate`/`-max_playback_rate` are 4.4 dash no-ops** — the muxer accepts every one of them but writes no `updatePeriod`/`Latency`/`PlaybackRate` into the manifest; live-latency hints need a newer ffmpeg, so ffkit doesn't expose them.
- **`remux --start-pid`/`--pmt-pid` verify via `probe.streams[].stream_id`** — mpegts PIDs come back as hex (`0x200`); `probe.programs[]` reports program num + SDT labels but never the PID map.
- **`+global_sidx` on the mov muxer is a 4.4 no-op** — the flag is accepted but no `sidx` box is written (the dash muxer's `-global_sidx` is the real one); ffkit doesn't expose it on `remux`.
- **`hls --live` already writes `EXT-X-PLAYLIST-TYPE:EVENT`** — plain packs write VOD, `--live` writes EVENT, so ffkit ships no `--vod`/`--event` flags: the tag is already correct by mode.
- **A framemd5 listing is text, not media** — `transcode --preset framemd5` writes `#format: frame checksums` + per-frame MD5 rows that ffprobe can't read back; the job reports via `write_job_raw` (existence + non-empty), and verification means diffing two listings, not probing one.
- **ffmpeg 4.4's jacosub demuxer garbles real .jss files** — it reads cues but emits junk text, so `subs --convert .jss` uses ffkit's own parser (digit-leading `HH:MM:SS.CC` cue lines, centisecond precision; `{...}` braces stripped, `|` → newline, directive lines skipped).

- **A `-media_seg_name` template without `$RepresentationID$` collides when the pack has more than one representation** — both streams write `chunk-001.m4s`, the rename fails mid-mux and the .mpd never lands. Plain `dash --init`/`--seg-name` names gain the rep/number variables automatically; only a raw `$` template can shoot itself in the foot.
- **The hls muxer won't create a missing output dir, and `-hls_flags append_list` rewrites the playlist in place** — `hls --append` needs the directory to exist and extends EXTINF rows only; segment numbering still restarts at 0, so pair it with `--start`/`--epoch`/`--time-names` to keep segment files distinct across runs.

- **`strftime_mkdir` is a standalone muxer option, not an `hls_flags` value** — `-hls_flags strftime_mkdir` fails with "Unable to parse option value" (it isn't in the flags list); it takes `-strftime_mkdir 1` beside `-strftime 1`. Only the *dir-creating* strftime path mkdirs — a `/%Y/` in a filename without it still fails on the missing dir (same as the plain-template no-mkdir rule).
- **`--time-dirs` hides segments one level deep** — `hls --time-names --time-dirs` writes `seg_YYYYMMDD/HHMMSS.ts`, so a top-level `ls` of the output dir shows **zero** `.ts` files even when the pack succeeded (ffkit's segment counter recurses for this). Anything that globs `dir/seg_*.ts` must walk the dated subdirs.
- **`<SegmentTimeline>` isn't optional for every player** — `-use_timeline 0` (ffkit `dash --no-timeline`) gives a template-only manifest older/basic players can parse; the default timeline form is required for precise segment addressing, so keep it on unless the target actually chokes.
- **Podlove `.psc` vs podcast JSON chapters** — same `chapters[]` container, different field: PSC uses `"start":"HH:MM:SS.mmm"`, the podcast JSON uses numeric `"startTime"`. ffkit's `--import` accepts both spellings plus a plain-seconds string for `start`.
- **`second_level_segment_index` needs a `%%`-escaped index slot.** The HLS segment template goes through strftime first — a bare `%03d` gets eaten by the clock expansion and the muxer then errors "Invalid second level segment filename template". ffkit appends `_%%03d` for `--time-names --seg-index`, which survives strftime as `%03d`.
- **`-mpegts_flags` is single-valued and `+`-joins.** Two separate `-mpegts_flags` args overwrite each other — `resend_headers`+`latm` must ride in one `resend_headers+latm` value (same rule as `-hls_flags`).
- **The `signature` fingerprint needs ≥1.5s of video** — identical clips under that window still report "no matching" (the MPEG-7 signature bank needs enough frames to fill), so `scan --dupe` on sub-1.5s social clips is a false negative, not a clean bill. Related: 4.4's `aphaseshift` ships only `shift`+`level` (its `order`/`type` options arrive in ffmpeg 6+).
- **`colorcorrect` on 4.4 is two-axis, not full RGB** — only `rl`/`bl` (shadow) + `rh`/`bh` (highlight) spots exist; there is no green pair, so `grade --shadows/--highlights` corrects along the red↔cyan + blue↔yellow axes only.
- **`silencedetect` floors ~13 dB under `volumedetect`'s `mean_volume` on tones** — a sine gap measuring -40 dB by volumedetect still trips `silencedetect=noise=-50dB` (it reads a per-sample window, not the clip mean). Calibrate `chapter --thresh` empirically against the real transcript's noise floor, not volumedetect's number.
...
- **The `dash` muxer can't embed text subtitles on 4.4** — the mp4 submuxer inside `-f dash` has no wvtt codec tag: `-c:s webvtt` fails with "Could not find tag for codec webvtt in stream #0" and `-tag:s wvtt` does not rescue it. fMP4/CMAF caption delivery stays HLS-side (`hls --subs`) or burned text.
- **`setsar`/`setdar` take fractions, not `N:D`** — `:` is the filter option separator, so `setdar=dar=16:9` misparses (produces sar=27:4 on 320x240 instead of dar=16:9). Write `N/D` — `setsar=4/3`, `setdar=16/9`; ffkit's `conform --sar/--dar` rewrites the colon input itself.
- **tee muxer slaves refuse mkv/webm.** `-f tee` on 4.4 writes mp4/mov/ts/m2ts/flv/mp3/wav/aac/flac/ogg slaves fine, but `[f=matroska]` fails "error writing header: Invalid data" and `[f=webm]` dies at the codec gate — `remux --also` refuses both up front. Top-level muxer opts (`-movflags`, `-write_prft`, `-video_track_timescale`, `-mpegts_*`) forward to **every** slave, so container-family flags can't combine with `--also` — ffkit refuses rather than write a broken pair.
- **`-write_prft` only lives in the mp4/mov muxer.** It emits a `prft` box per fragment under `frag_keyframe` (LL-DASH/CMAF ingest latency). The *dash* muxer also accepts a `write_prft` option on 4.4 — and writes nothing; a dead flag ffkit doesn't expose. `ffprobe` doesn't surface the box: byte-check `prft`.
- **`+write_colr` forces a `colr` atom even when colour metadata is fully unspecified** — platform QC that requires the atom regardless. mp4/mov/m4a targets only; probe reads nothing back (no colour tags to report), byte-check `colr`.
- `-cluster_time_limit` takes **milliseconds** (not the µs other muxer duration options use) — `remux --cluster 200` = 200ms. Passing microseconds (200000) silently makes clusters 200s — fewer than the default.
- `-frag_duration`/`-frag_size` only fire under `--frag` (mp4/mov muxer options, silent no-ops otherwise) — and fragment boundaries still snap to keyframes; the caps add *earlier* moofs, never mid-GOP ones.
- `-movflags +write_gama` writes no atom on 4.4 (dead flag); `+dash` writes no styp/msix on a non-fragmented single file; `+disable_chpl` only drops the QT chapter track — `-map_chapters` still copies chapters, so `--no-chapters` stays the real answer.
...
- `-adaptation_sets` `streams=` takes **output-stream indices or type groups** (`id=0,streams=0 id=1,streams=1`, or `streams=v`/`streams=a` to group a whole kind) — the `a:0` per-type syntax from `-var_stream_map` is NOT valid there ("already assigned to an AdaptationSet"/bad spec). `dash --var-map` maps `0:v?` then `0:a?`, so index 0 is the first video and audio indices follow in file order.
- `-hls_flags second_level_segment_size` is **broken on 4.4**: it only pairs with `-strftime 1`, and both the `%04d` and `%%04d` template forms get rejected by the muxer ("Cannot allocate memory"/"Invalid argument"). `hls --seg-index` (`second_level_segment_index`) is the sibling flag that works.
- `-min_seg_duration` is deprecated on the 4.4 dash muxer (warns "deprecated and will be removed") — `-seg_duration` is the live option; ffkit doesn't expose it.
- `-metadata isrc/license/publisher/barcode/catalognumber` land on mp3/flac/mkv/ogg but the mp4/m4a/mov whitelist **drops them silently** — same pattern as `make`/`model` (.mov-only keys). Tag music releases on the open containers, or carry the data in `description`/`copyright` for mp4 packs.
- `+default_base_moof` already ships inside `remux --frag` (the `frag_keyframe+empty_moov+default_base_moof` set — required for valid fMP4); no separate flag needed. `transcode --preset hevc` likewise auto-tags `hvc1`, so no `--tag` flag exists there — `remux --tag` retags on copy.
- `remux` to `.mp3` from a VIDEO source needs `--audio` (the mp3 muxer requires exactly one audio stream and can't hold the h264+aac copy pair — `-map 0` fails "Exactly one MP3 audio stream is required").
- `-use_template 0` on the dash muxer swaps the whole index style — no `<SegmentTemplate>` at all, each Representation carries `<SegmentList>`+`<SegmentURL>` entries instead (the `init-N`/`seg-N-NNNNN` file naming is unchanged). That's `dash --segment-list` for old/basic DASH players.
- `-mov_gamma 2.2` writes **zero** `gama` atoms on 4.4 — same dead-flag family as `+write_gama`; the mov muxer accepts the option and emits nothing. `-use_editlist 1` is likewise a no-op on a copy remux — the muxer already writes `elst` automatically when the source carries edit history.
- `+empty_moov` alone gives the *same* moof cadence as `frag_keyframe` — the moof multiplier is `+frag_every_frame` (`remux --frag-frame`), not `empty_moov` (which `--frag` already ships).
- On the 4.4 dash muxer `-min_buffer_time` does not exist (not a real option) and `-ldash` on file output only flips `minBufferTime` in the manifest — no LL machinery materializes without a live edge (same family as `-lhls`). `-hls_delete_threshold` removes stale segments but leaves the directory thin — partial cleanup only on 4.4; ffkit exposes neither.
